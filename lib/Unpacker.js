...
class Unpacker {
    static unpackData(data, index, mask_size) {
    ...
    static transformCostToToken(costs, quantityMultiplier) {
    ...
    static _loadResourcesIdsAndValuesFromCost(ids, values, costsLength, costs, cummulativeResourceCount) {
        if (costsLength === 0)
    ...
    static _loadSingleCostIdsAndValues(cost, idx, ids, values) {
    ...
    static unpackSquad(packedSquad) {
    ...
    static unpackTroop(packedTroop) {
        ...
        return {
            ...
            building: CombatConstants_1.CombatConstants.TROOPS_PROPS.Building[troopName],
     ...
     * @returns
     */
    static _splitInt(value, n, base, bound, output) {
        if (n === 0) {
            if (!value.eq(0))
    ...
    static _toBigNumber(num) {
        return !(num instanceof ethers_1.BigNumber) ? ethers_1.BigNumber.from(num.toString()) : num;
    }
    static _populateCostWithResourceIds(ids, values, quantityMultiplier) {
        let cost = {};
